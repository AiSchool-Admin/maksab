# مكسب — Automated Harvesting Engine (AHE) v3.0
## محرك الحصاد الآلي — المواصفات التقنية الشاملة والنهائية
### الإصدار: 3.0 Final | مارس 2026
...
### Harvest Scope = المصدر + الفئة + الفئة الفرعية + المحافظة + المدينة + السعر + نوع المعلن + التكرار

```
النظام يعرف مسبقاً "أين يبحث" عبر نطاقات محددة:

  ═══ المعاملات الأساسية ═══
  ① المصدر (source_platform)    — دوبيزل | السوق المفتوح | هتلاقي
  ② الفئة (maksab_category)     — موبايلات | سيارات | عقارات | إلكترونيات | أثاث
  ③ المحافظة (governorate)      — القاهرة | الإسكندرية | الجيزة | ...
  ④ التكرار (interval)          — كل 60 | 120 | 360 | 1440 دقيقة
  
  ═══ المعاملات المتقدمة (اختيارية) ═══
  ⑤ المدينة (city)              — سيدي جابر | مدينة نصر | المعادي | ...
  ⑥ الفئة الفرعية (subcategory) — آيفون | سامسونج | مرسيدس | شقق | ...
  ⑦ نطاق السعر (price_min/max)  — من 10,000 إلى 50,000 ج.م
  ⑧ حالة المنتج (condition)     — جديد | مستعمل
  ⑨ نوع المعلن (target_seller)  — الكل | تجار | أفراد | موثقين | حيتان 🐋
  ⑩ نوع الإعلان (target_listing)— الكل | مميز | إيليت
  ⑪ المجموعة (scope_group)      — عام | استهداف حيتان | منتجات عالية القيمة
  ...
  نطاق مركّز:
    Scope #2: دوبيزل + موبايلات + آيفون + الإسكندرية + سيدي جابر + 10K-50K + كل 120 دقيقة
  
  نطاق حيتان:
    Scope #3: دوبيزل + موبايلات + القاهرة + تجار فقط + إعلانات مميزة + كل 120 دقيقة
...
CREATE TABLE ahe_scopes (
  ...
  -- ═══ المعاملات المتقدمة (v3) ═══
  
  -- الفئة الفرعية
  subcategory TEXT,       -- NULL = كل الفئات الفرعية | 'iphone' | 'samsung' | ...
  subcategory_ar TEXT,    -- 'آيفون' | 'سامسونج' (للعرض في Dashboard)
  
  -- نطاق السعر
  price_min INTEGER,      -- NULL = لا حد أدنى | 5000 = فوق 5,000 ج.م
  price_max INTEGER,      -- NULL = لا حد أقصى | 50000 = تحت 50,000 ج.م
  
  -- حالة المنتج
  product_condition TEXT,  -- NULL = الكل | 'new' = جديد | 'used' = مستعمل
  
  -- نوع المعلن المستهدف
  target_seller_type TEXT DEFAULT 'all',
    -- 'all'        = كل المعلنين (الافتراضي)
    -- 'business'   = تجار فقط (صاحب عمل موثق)
    -- 'individual' = أفراد فقط
    -- 'verified'   = موثقين فقط (أفراد أو تجار)
    -- 'whales'     = "الحيتان" (تجار مميزين — إيليت + موثق + 10+ إعلانات)
  
  -- نوع الإعلانات المستهدفة
  target_listing_type TEXT DEFAULT 'all',
    -- 'all'                = كل الإعلانات (الافتراضي)
    -- 'featured'           = المميزة فقط
    -- 'elite'              = إيليت فقط
    -- 'featured_and_elite' = المميزة + إيليت
  
  -- التصنيف والمجموعة
  scope_group TEXT DEFAULT 'general',
    -- 'general'       = نطاق عام (الافتراضي)
    -- 'whale_hunting'  = استهداف الحيتان
    -- 'high_value'     = منتجات عالية القيمة
    -- 'new_areas'      = مناطق جديدة (استكشاف)
    -- 'seasonal'       = موسمي (رمضان/أعياد)
  
  -- وصف الغرض من النطاق
  description TEXT,       -- "استهداف تجار الآيفون في سيدي جابر"
...
-- ═══ Seed: نطاقات الحيتان (استهداف التجار الكبار) ═══
...
(code, name, source_platform, maksab_category, governorate, base_url, 
 ...
 target_seller_type, target_listing_type, scope_group, description) VALUES

('dub_whales_phones_cairo', '🐋 حيتان موبايلات — القاهرة', 'dubizzle', 'phones', 'cairo',
 'https://www.dubizzle.com.eg/mobile-phones-tablets-accessories-numbers/mobile-phones/cairo/',
 120, 5, 10, false, 'whales', 'featured_and_elite', 'whale_hunting',
 'استهداف تجار الموبايلات الكبار في القاهرة'),

('dub_whales_phones_alex', '🐋 حيتان موبايلات — الإسكندرية', 'dubizzle', 'phones', 'alexandria',
 'https://www.dubizzle.com.eg/mobile-phones-tablets-accessories-numbers/mobile-phones/alexandria/',
 120, 5, 10, false, 'whales', 'featured_and_elite', 'whale_hunting',
 'استهداف تجار الموبايلات الكبار في الإسكندرية'),

('dub_whales_vehicles_cairo', '🐋 حيتان سيارات — القاهرة', 'dubizzle', 'vehicles', 'cairo',
 'https://www.dubizzle.com.eg/vehicles/cars-for-sale/cairo/',
 120, 5, 10, false, 'whales', 'featured_and_elite', 'whale_hunting',
 'استهداف معارض السيارات الكبيرة في القاهرة');

-- ═══ Seed: نطاقات "عالية القيمة" ═══
INSERT INTO ahe_scopes 
(code, name, source_platform, maksab_category, governorate, base_url,
 harvest_interval_minutes, max_pages_per_harvest, priority, is_active,
 subcategory, subcategory_ar, scope_group, description) VALUES

('dub_iphone_cairo', '📱 آيفون — القاهرة', 'dubizzle', 'phones', 'cairo',
 'https://www.dubizzle.com.eg/mobile-phones-tablets-accessories-numbers/mobile-phones/cairo/?q=%D8%A2%D9%8A%D9%81%D9%88%D9%86',
 120, 3, 8, false, 'iphone', 'آيفون', 'high_value', 'آيفونات القاهرة — أعلى قيمة'),

('dub_iphone_alex', '📱 آيفون — الإسكندرية', 'dubizzle', 'phones', 'alexandria',
 'https://www.dubizzle.com.eg/mobile-phones-tablets-accessories-numbers/mobile-phones/alexandria/?q=%D8%A2%D9%8A%D9%81%D9%88%D9%86',
 120, 3, 8, false, 'iphone', 'آيفون', 'high_value', 'آيفونات الإسكندرية'),

('dub_samsung_cairo', '📱 سامسونج — القاهرة', 'dubizzle', 'phones', 'cairo',
 'https://www.dubizzle.com.eg/mobile-phones-tablets-accessories-numbers/mobile-phones/cairo/?q=%D8%B3%D8%A7%D9%85%D8%B3%D9%88%D9%86%D8%AC',
 120, 3, 7, false, 'samsung', 'سامسونج', 'high_value', 'سامسونج القاهرة');
```
...
CREATE TABLE ahe_sellers (
  ...
  whale_score INTEGER DEFAULT 0,
    -- Whale Score = مجموع:
    --   is_business × 30 | is_verified × 20 | has_featured_listings × 20
    --   total_listings > 5 × 10 | > 10 × 10 | > 20 × 10
    --   المجموع: 0-100 | حوت لو >= 60
  whale_detected_at TIMESTAMPTZ,
...
```sql
-- ════════════════════════════════════════════
-- الفئات الفرعية لكل فئة رئيسية (v3)
-- تُستخدم في Wizard الإضافة + بناء الـ URL
-- ════════════════════════════════════════════
CREATE TABLE IF NOT EXISTS ahe_subcategory_mappings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  maksab_category TEXT NOT NULL,       -- 'phones' | 'vehicles' | ...
  subcategory TEXT NOT NULL,           -- 'iphone' | 'samsung' | ...
  subcategory_ar TEXT NOT NULL,        -- 'آيفون' | 'سامسونج'
  source_platform TEXT NOT NULL,       -- 'dubizzle'
  source_query TEXT NOT NULL,          -- 'آيفون' (يُستخدم في ?q=)
  estimated_listings INTEGER DEFAULT 50,
...
-- ═══ Seed: فئات فرعية للموبايلات ═══
INSERT INTO ahe_subcategory_mappings 
(maksab_category, subcategory, subcategory_ar, source_platform, source_query, estimated_listings, display_order) VALUES
('phones', 'iphone', 'آيفون', 'dubizzle', 'آيفون', 200, 1),
('phones', 'samsung', 'سامسونج', 'dubizzle', 'سامسونج', 150, 2),
('phones', 'xiaomi', 'شاومي', 'dubizzle', 'شاومي', 100, 3),
('phones', 'oppo', 'أوبو', 'dubizzle', 'اوبو', 80, 4),
('phones', 'huawei', 'هواوي', 'dubizzle', 'هواوي', 60, 5),
('phones', 'realme', 'ريلمي', 'dubizzle', 'ريلمى', 60, 6),
('phones', 'honor', 'هونر', 'dubizzle', 'هونر', 50, 7),
('phones', 'infinix', 'إنفنيكس', 'dubizzle', 'إنفنيكس', 40, 8),
('phones', 'vivo', 'فيفو', 'dubizzle', 'فيفو', 30, 9),
('phones', 'nokia', 'نوكيا', 'dubizzle', 'نوكيا', 20, 10),
('phones', 'tecno', 'تكنو', 'dubizzle', 'تكنو', 25, 11),
-- ═══ Seed: فئات فرعية للسيارات ═══
('vehicles', 'mercedes', 'مرسيدس', 'dubizzle', 'مرسيدس', 100, 1),
('vehicles', 'bmw', 'بي إم دبليو', 'dubizzle', 'بي ام', 80, 2),
('vehicles', 'toyota', 'تويوتا', 'dubizzle', 'تويوتا', 120, 3),
('vehicles', 'hyundai', 'هيونداي', 'dubizzle', 'هيونداي', 100, 4),
('vehicles', 'kia', 'كيا', 'dubizzle', 'كيا', 80, 5),
('vehicles', 'nissan', 'نيسان', 'dubizzle', 'نيسان', 70, 6),
('vehicles', 'chevrolet', 'شيفروليه', 'dubizzle', 'شيفروليه', 60, 7),
-- ═══ Seed: فئات فرعية للعقارات ═══
('properties', 'apartments_sale', 'شقق للبيع', 'dubizzle', 'شقق للبيع', 200, 1),
('properties', 'apartments_rent', 'شقق للإيجار', 'dubizzle', 'شقق للإيجار', 150, 2),
('properties', 'villas', 'فيلات', 'dubizzle', 'فيلات', 50, 3),
('properties', 'land', 'أراضي', 'dubizzle', 'أراضي', 40, 4),
('properties', 'shops', 'محلات', 'dubizzle', 'محلات', 60, 5),
-- ═══ Seed: فئات فرعية للإلكترونيات ═══
('electronics', 'laptop', 'لابتوب', 'dubizzle', 'لابتوب', 100, 1),
('electronics', 'tv', 'تلفزيون', 'dubizzle', 'تلفزيون', 60, 2),
('electronics', 'playstation', 'بلايستيشن', 'dubizzle', 'بلايستيشن', 80, 3),
('electronics', 'tablet', 'تابلت', 'dubizzle', 'تابلت', 50, 4);
```
...
═══ المرحلة 5.5: FILTER BY SCOPE (v3 — فلترة متقدمة) ═══

  // بعض المعاملات لا تُطبق في الـ URL — تُطبق بعد الجلب
  
  IF scope.target_seller_type != 'all' OR scope.target_listing_type != 'all':
    
    لكل إعلان جديد:
      SWITCH scope.target_seller_type:
        'business'   → IF NOT listing.isBusiness → skip
        'individual' → IF listing.isBusiness → skip
        'verified'   → IF NOT listing.isVerified → skip
        'whales'     → IF NOT (listing.isBusiness AND listing.isVerified AND listing.isFeatured) → skip
      
      SWITCH scope.target_listing_type:
        'featured'           → IF NOT listing.isFeatured → skip
        'elite'              → IF NOT listing.isElite → skip
        'featured_and_elite' → IF NOT (listing.isFeatured OR listing.isElite) → skip
    
    // backup: فلتر السعر (لو URL filter ما اشتغلش)
    IF scope.price_min AND listing.price < scope.price_min → skip
    IF scope.price_max AND listing.price > scope.price_max → skip


═══ المرحلة 5.6: WHALE DETECTION (v3 — اكتشاف الحيتان) ═══

  // بعد كل حصادة — حدّث تصنيف كل معلن
  
  لكل seller تم تحديثه في هذه الحصادة:
    score = 0
    IF seller.is_business      → score += 30
    IF seller.is_verified      → score += 20
    IF seller.has_featured     → score += 20
    IF seller.total_listings > 5  → score += 10
    IF seller.total_listings > 10 → score += 10
    IF seller.total_listings > 20 → score += 10
    ...
    UPDATE ahe_sellers SET 
      whale_score = score,
      is_whale = is_whale,
      whale_detected_at = CASE WHEN is_whale AND NOT old.is_whale THEN now() ELSE old.whale_detected_at END
...
## 🔧 بناء URL النطاق مع المعاملات المتقدمة (v3)

```javascript
function buildScopeUrl(scope) {
  let url = scope.base_url;
  
  // المدينة — تستبدل المحافظة في الـ URL
  if (scope.city) {
    const cityMapping = getCityMapping(scope.city, scope.source_platform);
    if (cityMapping) url = url.replace('/' + scope.governorate + '/', '/' + cityMapping.source_url_segment + '/');
  }
  
  // Query Parameters
  const params = new URLSearchParams();
  if (scope.subcategory_ar) params.set('q', scope.subcategory_ar);
  if (scope.price_min) params.set('price_min', scope.price_min);
  if (scope.price_max) params.set('price_max', scope.price_max);
  if (scope.product_condition === 'new') params.set('condition', 'new');
  if (scope.product_condition === 'used') params.set('condition', 'used');
...
═══ الخطوة 1: المصدر ═══
  [دوبيزل 🟢] [السوق المفتوح ⚪] [هتلاقي ⚪]

═══ الخطوة 2: الفئة ═══
  الفئة الرئيسية: [موبايلات ▼]
  الفئة الفرعية (اختياري): [آيفون ▼]
    الخيارات من ahe_subcategory_mappings حسب الفئة الرئيسية

═══ الخطوة 3: الموقع ═══
  المحافظة: [الإسكندرية ▼]
  المدينة (اختياري): [سيدي جابر ▼]

═══ الخطوة 4: الفلاتر المتقدمة (اختياري — collapsible) ═══
  
  نطاق السعر:
    من: [______] ج.م    إلى: [______] ج.م
    اختصارات: [أقل من 5K] [5K-20K] [20K-50K] [أكتر من 50K]
  
  حالة المنتج:
    [○ الكل] [○ جديد] [○ مستعمل]
  
  نوع المعلن المستهدف:
    [○ الكل] [○ تجار فقط] [○ أفراد فقط] [○ موثقين فقط] [○ حيتان 🐋]
    ⓘ "حيتان" = تجار موثقين + إعلانات مميزة + أكتر من 10 إعلانات
  
  نوع الإعلانات:
    [○ الكل] [○ مميز فقط] [○ إيليت فقط]
  
  المجموعة:
    [○ عام] [○ استهداف حيتان] [○ منتجات عالية القيمة]
  
  وصف (اختياري):
    [استهداف تجار الآيفون في سيدي جابر — عملاء لباقة Diamond]
...
═══ الملخص ═══
  الرابط يُبنى تلقائياً من المعاملات
  ✅ تم التحقق — الرابط صالح
  [💾 حفظ وتفعيل] [💾 حفظ متوقف] [🧪 تجريبي]
```
...
│ المجموعة: عام | النوع: كل المعلنين                      │
│                                                            │
...
│ ═══ الفلاتر المتقدمة (v3) ═══                            │
│ الفئة الفرعية: [الكل ▼]                                  │
│ نطاق السعر: من [___] إلى [___] ج.م                      │
│ حالة المنتج: [○ الكل] [○ جديد] [○ مستعمل]              │
│ نوع المعلن: [○ الكل] [○ تجار] [○ أفراد] [○ حيتان 🐋]  │
│ نوع الإعلانات: [○ الكل] [○ مميز] [○ إيليت]             │
│ المجموعة: [عام ▼]                                        │
...
│ حيتان جدد: 5 🐋 | متوسط/ساعة: 18 | أخطاء: 0             │
│                                                            │
│ [💾 حفظ التعديلات]                                       │
└──────────────────────────────────────────────────────────┘

تبويبات المجموعات فوق جدول النطاقات:
  [الكل (21)] [عام (15)] [حيتان 🐋 (3)] [عالي القيمة (3)]

أعمدة إضافية في الجدول:
  | المجموعة | نوع المعلن | الفلاتر | حيتان 🐋 |
```
...
/lib/harvester
  ...
  url-builder.ts                      — بناء الروابط من Mapping Tables + المعاملات المتقدمة
  location-mapper.ts                  — ترجمة الموقع من نص المنصة لمعيار مكسب
  scope-filter.ts                     — فلترة ما بعد الجلب (نوع المعلن/الإعلان)
  whale-detector.ts                   — اكتشاف وتصنيف الحيتان (Whale Score)
...
/migrations
  ...
  ahe_005_subcategory_mappings.sql    — جدول الفئات الفرعية + Seed (v3)
  ahe_006_advanced_scope_params.sql   — أعمدة متقدمة لـ ahe_scopes (v3)
  ahe_007_whale_fields.sql            — حقول الحيتان في ahe_sellers (v3)
  ahe_008_seed_whale_scopes.sql       — نطاقات الحيتان + عالية القيمة (v3)
```
...
### ═══ المرحلة 3: الصلاحية + التوسع + المعاملات المتقدمة ═══
...
═══ المعاملات المتقدمة (v3) ═══
☐ Migration: ahe_subcategory_mappings + Seed (45 فئة فرعية)
☐ Migration: أعمدة متقدمة لـ ahe_scopes (subcategory, price, seller_type, etc.)
☐ Migration: whale fields لـ ahe_sellers (is_whale, whale_score)
☐ Migration: Seed نطاقات الحيتان + عالية القيمة (6 نطاقات)
☐ lib/harvester/scope-filter.ts (فلترة بعد الجلب)
☐ lib/harvester/whale-detector.ts (Whale Score 0-100)
☐ تعديل url-builder.ts لدعم subcategory + price في URL
☐ تعديل engine.ts لتطبيق filterByScope + updateWhaleStatus
☐ /admin/crm/harvester/scopes — تبويبات المجموعات + أعمدة متقدمة
☐ /admin/crm/harvester/scopes/new — Wizard 5 خطوات مع فلاتر متقدمة
☐ /admin/crm/harvester/scopes/{id} — Modal إعدادات محدّث
☐ Parsers إضافية: opensooq.ts + hatla2ee.ts (+ Mappings لهم)
☐ اختبار: أسبوع كامل + تقرير أداء شامل + صلاحية + حيتان
```
