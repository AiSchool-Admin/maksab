  ...
  const handlePhoneSubmit = async () => {
    ...
    if (otpResult.channel === "firebase" && isFirebaseConfigured()) {
      ...
      try {
        ...
        const recaptchaReady = await setupRecaptcha("recaptcha-container-sheet");
        if (!recaptchaReady) {
          console.warn("[Auth] reCAPTCHA setup failed — falling back to dev mode");
        } else {
          const firebaseResult = await sendFirebaseOTP(normalizedPhone);
      ...
      // If Firebase failed, fall back to dev mode (re-send with forced dev channel)
      if (!firebaseSuccess) {
        const devFallback = await sendOTP(normalizedPhone, true);
