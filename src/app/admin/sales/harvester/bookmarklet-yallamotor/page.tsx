...
export default function YallamotorBookmarkletPage() {
  ...
  const bookmarkletCode = appUrl ? buildYallamotorBookmarklet(appUrl) : "";
    ...
    <div className="space-y-6 max-w-4xl mx-auto" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            🚗 Bookmarklet — حصاد يلا موتور
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            استخراج إعلانات السيارات من yallamotor.com — محظور من السيرفر (403)
          </p>
        </div>
        <Link
      ...
      <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
        ...
        <p className="text-sm text-red-600">
          Yallamotor بيحظر الطلبات من سيرفرات Cloud بكود 403.
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5">
        ...
        <ol className="space-y-2 text-sm text-gray-700">
          ...
          <li className="flex gap-2">
            ...
            <span>افتح صفحة سيارات في <code className="bg-blue-100 px-1 rounded" dir="ltr">yallamotor.com</code></span>
          </li>
          <li className="flex gap-2">
      ...
      <div className="bg-white border rounded-2xl p-5">
        ...
        <button
          ...
          className={`px-6 py-3 rounded-xl font-bold text-lg mb-3 transition-colors ${
            codeCopied ? "bg-green-600 text-white" : "bg-blue-600 text-white hover:bg-blue-700"
          }`}
        >
...
function buildYallamotorBookmarklet(appUrl: string): string {
  const code = `
(function(){
var MAKSAB='${appUrl}';
var PLATFORM='yallamotor';

if(!window.location.hostname.includes('yallamotor')){
  alert('\\u274C هذا الـ Bookmarklet خاص بموقع Yallamotor\\n\\nافتح yallamotor.com وجرب تاني');
  return;
}
...
function extractListings(){
  ...
  var selectors=[
    'a[href*="/car/"]','a[href*="/used-cars/"]','a[href*="/new-cars/"]',
    '[class*="car-card"]','[class*="listing-card"]','[class*="vehicle-card"]',
    '[class*="CarCard"]','[class*="ListingCard"]',
    '.search-result-item','[data-testid*="car"]','[data-testid*="listing"]'
  ];
  var cards=document.querySelectorAll(selectors.join(','));
  console.log('Maksab Yallamotor: Found',cards.length,'potential cards');

  for(var i=0;i<cards.length;i++){
    var el=cards[i];
    var a=el.tagName==='A'?el:el.querySelector('a[href*="/car/"],a[href*="/used"],a[href]');
    if(!a)continue;
    var url=a.href;
    if(!url||seenUrls[url])continue;
    if(url.includes('/search')||url.includes('/compare')||url.includes('/login'))continue;
    if(!/yallamotor/.test(url))continue;
    seenUrls[url]=true;

    var card=a.closest('[class*=card],[class*=listing],[class*=vehicle],[class*=item],[class*=result]')||a.parentElement.parentElement||a;
    var cardText=card.textContent||'';

    var title=a.getAttribute('title')||'';
    if(!title||title.length<3){
      var h=card.querySelector('h2,h3,h4,[class*=title],[class*=name],[class*=Title]');
    ...
    if(!price){
      ...
      if(priceEl){var pt=priceEl.textContent.replace(/[^\\d]/g,'');if(pt&&pt.length>3)price=parseInt(pt);}
    }
    ...
    var sellerEl=card.querySelector('[class*=dealer],[class*=seller],[class*=shop]');
    ...
    listings.push({
      ...
      thumbnailUrl:thumbnail,location:location,dateText:'',
      sellerName:sellerName,sellerProfileUrl:null,
      isVerified:isDealer,isBusiness:isDealer,
  ...
  console.log('Maksab Yallamotor: Extracted',listings.length,'listings');
  return listings;
}

var listings=extractListings();
if(listings.length===0){
  alert('لم يتم العثور على إعلانات\\n\\nتأكد إنك على صفحة قوائم سيارات في Yallamotor');
  return;
}
...
var checkReady=setInterval(function(){
  try{popup.postMessage({type:'harvest_data',payload:payload,token:'bookmarklet-yallamotor'},MAKSAB);}catch(e){}
},500);
